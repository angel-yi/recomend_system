...
ESUNChart.on = function (o, type, fn) {
...
JoinLine.prototype = {
    ...
    pos: function (obj) {
        ...
        for (; a; a = a.offsetParent) {
            pos.x += a.offsetLeft;
            pos.y += a.offsetTop;
            if (this.wrap && a.offsetParent === this.wrap) break
        }
        ;// 兼容居中div
    ...
    _oldLine: function (x1, y1, x2, y2) {
        ...
        do {
            this.FFDot(x1 + i * dx, y1 + i * dy, this.color, this.size)
        } while (i++ < r);
    },
...
oZXZ = {
    ...
    bind: function (tid, _css_name, _on_off) {
        this.table = tid;
        for (var i = 0; i < _css_name.length; i++) {
            this.css_name.push(_css_name[i])
        }
        ;
        this.on_off = _on_off;
        return this;
    ...
    draw: function (yes) {
        ...
        if (yes) {
            ...
            for (var i = 0; i < qL; i++) {
                ...
                this.novl = it.novl;
                if (this.novl) LG.filter = function (a, b) {
                    return !(a.x == b.x)
                };
